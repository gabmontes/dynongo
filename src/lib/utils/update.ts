...
export function parse(query: UpdateQuery): ParseResult {
	...
	if (query.$set) {
		...
		expr.set = expr.set.concat(
			Object.keys(query.$set).map((key) => {
				const value = query.$set![key];
				if (value?.$ifNotExists) {
					const k = nameUtil.generateKeyName(key);
					const v = nameUtil.generateValueName(
						key,
						value.$ifNotExists,
						values,
						true
	...
	if (query.$unset) {
		...
		expr.remove = expr.remove.concat(
			Object.keys(query.$unset).map((key) => {
				const k = nameUtil.generateKeyName(key);
	...
	if (query.$inc) {
		...
		expr.add = expr.add.concat(
			Object.keys(query.$inc).map((key) => {
	...
	if (query.$push || query.$unshift) {
		...
		expr.set = expr.set.concat(
			Object.keys(query[operator]!).map((key) => {
	...
	if (query.$addToSet) {
		...
		expr.add = expr.add.concat(
			Object.keys(query.$addToSet).map((key) => {
				const value = fromArrayEach(query.$addToSet![key], false);
				const set = new Set(value);
				const k = nameUtil.generateKeyName(key);
				const v = nameUtil.generateValueName(key, set, values, true);
	...
	if (query.$removeFromSet) {
		...
		expr.delete = expr.delete.concat(
			Object.keys(query.$removeFromSet).map((key) => {
