...
test("$set", (t) => {
	...
	t.is(
		...
		"SET #k_id=:v_id, #k_description=:v_description"
	);
	t.deepEqual(result.ExpressionAttributeNames, {
		"#k_id": "id",
...
test("$set $ifNotExists", (t) => {
	const result = update.parse({
		$set: { id: 5, description: { $ifNotExists: "foo" } },
	});

	t.is(
		result.UpdateExpression,
		"SET #k_id=:v_id, #k_description=if_not_exists(#k_description, :v_description)"
	);
	t.deepEqual(result.ExpressionAttributeNames, {
		"#k_id": "id",
		"#k_description": "description",
...
test("$push", (t) => {
	...
	t.is(
		result.UpdateExpression,
		"SET #k_scores=list_append(if_not_exists(#k_scores, :_v_empty_list), :v_scores)"
	);
...
test("$unshift", (t) => {
	...
	t.is(
		result.UpdateExpression,
		"SET #k_scores=list_append(:v_scores, if_not_exists(#k_scores, :_v_empty_list))"
	);
	t.deepEqual(result.ExpressionAttributeNames, { "#k_scores": "scores" });
	t.deepEqual(result.ExpressionAttributeValues, {
		":v_scores": [85],
...
test("$addToSet array", (t) => {
	...
	t.deepEqual(result.ExpressionAttributeValues, {
		...
		":v_friends": marshall(["mario", "luigi"]),
	});
});

test("$removeFromSet array", (t) => {
	const result = update.parse({
		$removeFromSet: { friends: ["mario", "luigi"] },
	});
...
test("$addToSet and $inc", (t) => {
	const result = update.parse({
		$addToSet: { friends: "mario" },
		$inc: { number: 1 },
	});

	t.is(
		result.UpdateExpression,
		"ADD #k_number :v_number, #k_friends :v_friends"
	);
	t.deepEqual(result.ExpressionAttributeNames, {
		"#k_number": "number",
		"#k_friends": "friends",
...
test("$addToSet $each in array", (t) => {
	const result = update.parse({
		$addToSet: { friends: { $each: ["mario", "luigi"] } },
	});
...
test("$addToSet (double)", (t) => {
	const result = update.parse({
		$addToSet: { friends: { $each: ["mario", "luigi"] }, enemies: "bowser" },
	});

	t.is(
		result.UpdateExpression,
		"ADD #k_friends :v_friends, #k_enemies :v_enemies"
	);
	t.deepEqual(result.ExpressionAttributeNames, {
		"#k_friends": "friends",
		"#k_enemies": "enemies",
	...
	t.deepEqual(result.ExpressionAttributeValues, {
		...
		":v_friends": marshall(["mario", "luigi"]),
		":v_enemies": marshall(["bowser"]),
	});
});

test("$removeFromSet $each in array", (t) => {
	const result = update.parse({
		$removeFromSet: { friends: { $each: ["mario", "luigi"] } },
	});
...
test("$removeFromSet (double)", (t) => {
	...
	t.is(
		result.UpdateExpression,
		"DELETE #k_friends :v_friends, #k_enemies :v_enemies"
	);
	t.deepEqual(result.ExpressionAttributeNames, {
		"#k_friends": "friends",
		"#k_enemies": "enemies",
...
test("$push throws error if $each is not an array", (t) => {
	t.throws(
		update.parse.bind(update, { $push: { scores: { $each: 85 } } }),
		"The value for $each should be an array."
	);
});

test("$unshift throws error if $each is not an array", (t) => {
	t.throws(
		update.parse.bind(update, { $unshift: { scores: { $each: 85 } } }),
		"The value for $each should be an array."
	);
});
